...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    if (check_admin_referer('oee_delete_student_test_' . intval($_GET['id']))) {
        $del_id = intval($_GET['id']);
        $del_res = OEE_Student_Tests::delete($del_id);
        if (is_wp_error($del_res)) {
            echo '<div class="notice notice-error is-dismissible"><p>' . esc_html($del_res->get_error_message()) . '</p></div>';
        } else {
            echo '<div class="notice notice-success is-dismissible"><p>' . esc_html(olama_exam_translate('Student test deleted.')) . '</p></div>';
        }
    }
}
...
<div class="olama-exam-wrap" dir="rtl">
    <div class="olama-exam-header">
        <div>
            <h1><?php echo olama_exam_translate('student_tests_menu'); ?></h1>
        </div>
        <div class="actions">
            <a href="<?php echo admin_url('admin.php?page=oee-student-tests&action=add'); ?>" class="olama-exam-btn olama-exam-btn-primary">+ <?php echo olama_exam_translate('Add New'); ?></a>
        </div>
    </div>
    ...
    <div class="olama-exam-card">
        <table class="olama-exam-table">
            ...
            <tbody>
                <?php if (empty($tests)): ?>
                    <tr>
                        <td colspan="9"><?php echo olama_exam_translate('No student tests found.'); ?></td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($tests as $test): 
                        $public_url = home_url('/student-test/' . $test->public_token);
                        $config = json_decode($test->subject_config, true);
                        $total_q = 0;
                        if (is_array($config)) {
                            foreach ($config as $sub) {
                                $total_q += intval($sub['num_questions']);
                            }
                        }
                        ?>
                        <tr>
                            <td><strong><a href="<?php echo admin_url('admin.php?page=oee-student-tests&action=edit&id=' . $test->id); ?>"><?php echo esc_html($test->title); ?></a></strong></td>
                            <td><?php echo esc_html($test->grade_name_ar); ?></td>
                            ...
                            <td>
                                <a href="<?php echo admin_url('admin.php?page=oee-student-tests&action=edit&id=' . $test->id); ?>" class="olama-exam-btn olama-exam-btn-outline olama-exam-btn-sm">✏️ <?php echo olama_exam_translate('Edit'); ?></a>
                                <a href="<?php echo wp_nonce_url(admin_url('admin.php?page=oee-student-tests&action=delete&id=' . $test->id), 'oee_delete_student_test_' . $test->id); ?>" class="olama-exam-btn olama-exam-btn-danger olama-exam-btn-sm" onclick="return confirm('<?php echo esc_attr(olama_exam_translate('Are you sure you want to delete this test?')); ?>');">🗑 <?php echo olama_exam_translate('Delete'); ?></a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>

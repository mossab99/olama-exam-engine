...
<div class="olama-exam-wrap">
    ...
    <!-- Academic Context (read-only) + Grade/Subject Selectors or Profession Selector -->
    ...
    ?>
        <div class="olama-exam-card">
            <div class="olama-exam-card-header">
                <h3>💼 <?php echo olama_exam_translate('Select Profession'); ?></h3>
            </div>
            <div style="padding:20px;">
                <div class="olama-exam-form-row" style="grid-template-columns: 1fr;">
                    <div class="olama-exam-form-group">
                        ...
                        <select id="qb-profession-select">
                ...
                <div class="olama-exam-modal-body">
                    ...
                    <div class="olama-exam-form-row">
                        <?php if ($qb_type === 'grade_level'): ?>
                            <input type="hidden" name="category_id" id="q-category-id" value="0">
                        <?php else: ?>
                            <div class="olama-exam-form-group">
...
<script>
    (function ($) {
        ...
        $('#qb-grade-level-select').on('change', function () {
            ...
            $('#qb-active-unit-name').text(gradeLevelName);
        ...
        $('#qb-profession-select').on('change', function () {
            ...
            const professionName = $(this).find('option:selected').text();
            ...
            $('#qb-active-unit-name').text(professionName);
            $('#q-profession-id').val(professionId);
            $('#q-unit-id').val(0);
            $('#q-lesson-id').val(0);
...
1. اختيار من متعدد (Multiple Choice) - أربعة خيارات:
::عنوان السؤال::نص السؤال؟{=إجابة صحيحة ~خيار خاطئ 1 ~خيار خاطئ 2 ~خيار خاطئ 3}
...
- **لا تستخدم أي نوع آخر من الأسئلة غير المذكورة أعلاه.**
...
- وزع الأسئلة بين نوعي (الاختيار من متعدد) و (صح أو خطأ) بشكل متوازن.
...
🚀 ابدأ الآن.`;
        ...
        $('#qb-import-gift-btn').on('click', function () {
            if (qbType === 'profession') {
                ...
                const gradeLevelId = $('#qb-grade-level-select').val();
                window.location.href = `?page=olama-exam-import-gift&qb_type=grade_level&grade_level_id=${gradeLevelId}&category_id=0`;
            } else {
                const gradeId = $('#qb-grade-select').val();
        ...
        $('#qb-import-csv-btn').on('click', function () {
            if (qbType === 'profession') {
                ...
                const gradeLevelId = $('#qb-grade-level-select').val();
                window.location.href = `?page=olama-exam-import-csv&qb_type=grade_level&grade_level_id=${gradeLevelId}&category_id=0`;
            } else {
                const gradeId = $('#qb-grade-select').val();
                const subjectId = $('#qb-subject-select').val();
                const lessonId = $('#filter-lesson').val() || 0;
